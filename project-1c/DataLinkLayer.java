...
// =============================================================================
	...
	for (byte b : data) {
	    ...
	    if (debug) {
		printByte("DataLinkLayer.transmit(): Sending byte = ", b);
	    }
	...
	while (bitBuffer.size() >= Byte.SIZE) {
	    ...
	    if (debug) {
		printByte("DataLinkLayer.receive(): Got new byte = ", newByte);
	    }
    ...
    // =========================================================================
    /**
     * Emit a byte value to stdout.  Print it as an unsigned integer, a
     * hexidecimal value, and, if possible, as a printable character.
     *
     * @param lead  The leading string to print before the byte value.
     * @param value The byte value to print.
     */
    private static void printByte (String lead, byte value) {

	int i = value & 0xff;
	System.out.print(lead);
	System.out.printf("%d / 0x%x", i, i);
	if (i < 127) {
